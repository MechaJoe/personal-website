<script>
  ...
  onMount(() => {
    window.addEventListener("scroll", function () {
      ...
      showA = window.pageYOffset >= vpHeight * 0.75
      showB = window.pageYOffset >= vpHeight * 0.9
      showC = window.pageYOffset >= vpHeight * 1.25
    })
  })
</script>
...
<div class="h-fit w-fit flex py-20 px-10 justify-start">
  <div>
    ...
    <div class="top-80 grid grid-cols-2 gap-y-40">
      {#if showA}
        <h2 in:fade out:fade class="text-3xl">...an East Coaster</h2>
        <p>
          Joe was born in Pennsylvania and raised in Connecticut. Some of his
          favorite memories are of spending long, breezy summer days with his
          friends at Tod's Point.
        </p>
      {/if}
      {#if showB}
      ...
      {#if showC}
        <h2 in:fade out:fade class="text-3xl">
          ...a musician and cordial connoisseur
        </h2>
        <p>
          Growing up, Joe was classically trained to play the violin. He
